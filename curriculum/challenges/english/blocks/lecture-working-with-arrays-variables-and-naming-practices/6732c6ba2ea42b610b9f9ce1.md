...
In this example, we create a new array using `Array(5)`. This creates a sparse array with a length of 5. Sparse arrays have empty slots — positions that exist but do not hold any value. The interactive editor displays these empty slots as `undefined`, but these are not actual `undefined` values that were assigned, they are just uninitialized positions in the array.
